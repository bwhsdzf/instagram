...
import com.mobile.instagram.activities.User_Feed.ImagePagerActivity;
import com.mobile.instagram.activities.User_Feed.MyApplication;
import com.mobile.instagram.adapter.CommentAdapter.ICommentItemClickListener;
import com.mobile.instagram.bean.ActionItem;
...
import com.mobile.instagram.mvp.presenter.CirclePresenter;
import com.mobile.instagram.mvp.view.ICircleViewUpdate;
import com.mobile.instagram.spannable.ISpanClick;
import com.mobile.instagram.utils.DatasUtil;
import com.mobile.instagram.widgets.AppNoScrollerListView;
import com.mobile.instagram.widgets.CircularImage;
import com.mobile.instagram.widgets.FavortListView;
import com.mobile.instagram.widgets.MultiImageView;
import com.mobile.instagram.widgets.SnsPopupWindow;
import com.mobile.instagram.widgets.dialog.CommentDialog;
...
public class CircleAdapter extends BaseAdapter implements ICircleViewUpdate {
	...
	private CirclePublicCommentContral circlePublicCommentContral;
	private List<CircleItem> datas = new ArrayList<CircleItem>();

	public void setCirclePublicCommentContral(
			CirclePublicCommentContral circlePublicCommentContral) {
		this.circlePublicCommentContral = circlePublicCommentContral;
	}
	...
	public int getItemViewType(int position) {
		...
		CircleItem item = datas.get(position);/*if (ITEM_TYPE_URL.equals(item.getType())) {
			itemType = ITEM_VIEW_TYPE_URL;
		} else */
		if (ITEM_TYPE_IMAGE.equals(item.getType())) {
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if(convertView == null){
			...
			case ITEM_VIEW_TYPE_IMAGE:// 图片view
				...
				linkOrImgViewStub.inflate();
				MultiImageView multiImageView = (MultiImageView) convertView.findViewById(R.id.multiImagView);/*九宫格图片*/
		...
		String content = circleItem.getUser().getContent();
		String createTime = circleItem.getUser().getDate();
		String location=circleItem.getUser().getLocation();
				...
				holder.bbsAdapter.setCommentClickListener(new ICommentItemClickListener() {
					...
					public void onItemClick(int commentPosition) {
						...
						if(DatasUtil.curUser.getId().equals(commentItem.getUser().getId())){//复制或者删除自己的评论
							...
							if(circlePublicCommentContral !=null){
								circlePublicCommentContral.editTextBodyVisible(View.VISIBLE, mPresenter, position, TYPE_REPLY_COMMENT, commentItem.getUser(), commentPosition);
							}
						}
					}
				});
		...
		if(!TextUtils.isEmpty(curUserFavortId)){
			snsPopupWindow.getmActionItems().get(0).mTitle = "Cancel";
		}else{
			snsPopupWindow.getmActionItems().get(0).mTitle = "Like";
		}
		...
		holder.urlTipTv.setVisibility(View.GONE);/*有图片则没有Pulished a msg这句话*/
		switch (itemViewType) {
	...
	private class PopupItemClickListener implements SnsPopupWindow.OnItemClickListener{
		...
		public void onItemClick(ActionItem actionitem, int position) {
			...
			case 0://点赞、取消点赞
				...
				if ("Like".equals(actionitem.mTitle.toString())) {
			...
			case 1://发布评论
				if(circlePublicCommentContral !=null){
					circlePublicCommentContral.editTextBodyVisible(View.VISIBLE, mPresenter, mCirclePosition, TYPE_PUBLIC_COMMENT, null, 0);
				}
				break;
			default:
				break;
	...
	public void update2AddComment(int circlePosition, int type, User replyUser) {
		...
		String content = "";
		if(circlePublicCommentContral !=null){
			content = circlePublicCommentContral.getEditTextString();
		}
		...
		notifyDataSetChanged();
		if(circlePublicCommentContral !=null){
			circlePublicCommentContral.clearEditText();
		}
	}
